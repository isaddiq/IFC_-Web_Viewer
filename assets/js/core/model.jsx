/**
 * Synthetic IFC data model.
 *
 * In a real IFC viewer this would be produced by web-ifc parsing a real .ifc
 * file in a Web Worker. Here we generate a procedural 3-storey office building
 * and stamp every part with a realistic-looking IFC entity record so the UI
 * (inspector, tree, schema explorer, search) behaves exactly as it would on
 * real data.
 */
...
// Random-ish but deterministic helpers ---------------------------------------
...
const MATERIALS = {
  ...
  glass: { name: 'Glazing - Double Low-E', density: 2500, thermalConductivity: 1.0, layers: [
    { name: 'Glass outer', thickness: 6 }, { name: 'Argon cavity', thickness: 16 }, { name: 'Glass inner', thickness: 6 },
  ] },
...
// Build the procedural model -------------------------------------------------
function buildModel() {
  const elements = []; // flat list of IFCElement
  const byId = new Map(); // expressId -> element
  const relationships = []; // global rel list
    ...
    propertySets: [
      {
        ...
        properties: [
          ...
          { name: 'BuildableArea', value: 4200, unit: 'm²', type: 'IfcAreaMeasure' },
          { name: 'TotalArea', value: 5840, unit: 'm²', type: 'IfcAreaMeasure' },
        ],
      },
    ],
    quantitySets: [],
  ...
  const building = add({
    ...
    name: 'Block A',
    description: '3-storey office block, GFA approx 1,840 m².',
    objectType: null,
    predefinedType: null,
    ...
    propertySets: [
      {
        ...
        properties: [
          ...
          { name: 'GrossFloorArea', value: 1840, unit: 'm²', type: 'IfcAreaMeasure' },
  ...
  // ----- Type objects (shared by elements of the same kind) -----
  ...
  const types = {
    wallExt: typeObject('IfcWallType', 'WT-EXT-250', 'STANDARD', [
      { name: 'Pset_WallCommon', properties: [
        ...
        { name: 'ThermalTransmittance', value: 0.22, unit: 'W/m²K', type: 'IfcThermalTransmittanceMeasure' },
      ]},
    ]),
    ...
    window: typeObject('IfcWindowType', 'WT-EXT-W01', 'WINDOW', [
      { name: 'Pset_WindowCommon', properties: [
        ...
        { name: 'IsExternal', value: true, type: 'IfcBoolean' },
        { name: 'ThermalTransmittance', value: 1.1, unit: 'W/m²K', type: 'IfcThermalTransmittanceMeasure' },
    ...
    const s = add({
      ...
      propertySets: [
        { name: 'Pset_BuildingStoreyCommon', properties: [
          ...
          { name: 'GrossFloorArea', value: 612, unit: 'm²', type: 'IfcAreaMeasure' },
          { name: 'NetFloorArea', value: 540, unit: 'm²', type: 'IfcAreaMeasure' },
        ]},
      ],
      quantitySets: [
        { name: 'Qto_BuildingStoreyBaseQuantities', properties: [
          ...
          { name: 'NetVolume', value: 1944, unit: 'm³', type: 'IfcVolumeMeasure' },
        ]},
      ],
      materials: [],
      classifications: [],
  ...
  // ----- Per-storey elements -----
  // Footprint: rectangle 24m x 14m centred at origin, Z up.
  const W = 24, D = 14;
  ...
  storeys.forEach((storey, sIdx) => {
    ...
    const slab = add(makeElement({
      cls: 'IfcSlab', typeObj: types.slab, predefined: 'FLOOR',
      ...
      psets: [
        { name: 'Pset_SlabCommon', properties: [
          ...
          { name: 'PitchAngle', value: 0, unit: '°', type: 'IfcPlaneAngleMeasure' },
        ]},
      ],
      qtos: [
        { name: 'Qto_SlabBaseQuantities', properties: [
          { name: 'GrossArea', value: (W + 0.5) * (D + 0.5), unit: 'm²', type: 'IfcAreaMeasure' },
          { name: 'NetArea', value: (W + 0.5) * (D + 0.5) - 8.5, unit: 'm²', type: 'IfcAreaMeasure' },
          { name: 'GrossVolume', value: (W + 0.5) * (D + 0.5) * 0.2, unit: 'm³', type: 'IfcVolumeMeasure' },
    ...
    // ----- Exterior walls (4 walls forming perimeter) -----
    ...
    const extWallEls = extWalls.map(w => add(makeElement({
      cls: 'IfcWallStandardCase', typeObj: types.wallExt, predefined: 'STANDARD',
      ...
      qtos: [
        { name: 'Qto_WallBaseQuantities', properties: [
          ...
          { name: 'GrossSideArea', value: (w.w >= w.d ? w.w : w.d) * wallH, unit: 'm²', type: 'IfcAreaMeasure' },
          { name: 'NetVolume', value: (w.w >= w.d ? w.w : w.d) * wallH * wallT, unit: 'm³', type: 'IfcVolumeMeasure' },
        ]},
      ],
    })));

    // ----- Interior walls (split each storey into 3 zones) -----
    const intT = 0.1;
    const intWallA = add(makeElement({
      cls: 'IfcWallStandardCase', typeObj: types.wallInt, predefined: 'STANDARD',
    ...
    colXs.forEach(cx => colYs.forEach(cy => {
      const c = add(makeElement({
        cls: 'IfcColumn', typeObj: types.column, predefined: 'COLUMN',
        ...
        qtos: [{ name: 'Qto_ColumnBaseQuantities', properties: [
          { name: 'Length', value: wallH, unit: 'm', type: 'IfcLengthMeasure' },
          { name: 'CrossSectionArea', value: colSize * colSize, unit: 'm²', type: 'IfcAreaMeasure' },
          { name: 'OuterSurfaceArea', value: 4 * colSize * wallH, unit: 'm²', type: 'IfcAreaMeasure' },
          { name: 'GrossVolume', value: colSize * colSize * wallH, unit: 'm³', type: 'IfcVolumeMeasure' },
        ]},
        ],
      }));
      cols.push(c);
    ...
    // ----- Beams along x-axis at top of storey -----
    const beamH = 0.6;
    [-4.5, 4.5].forEach(by => {
      const b = add(makeElement({
        cls: 'IfcBeam', typeObj: types.beam, predefined: 'BEAM',
        ...
        qtos: [{ name: 'Qto_BeamBaseQuantities', properties: [
          { name: 'Length', value: W - 1.5, unit: 'm', type: 'IfcLengthMeasure' },
          { name: 'CrossSectionArea', value: 0.4 * beamH, unit: 'm²', type: 'IfcAreaMeasure' },
          { name: 'GrossVolume', value: (W - 1.5) * 0.4 * beamH, unit: 'm³', type: 'IfcVolumeMeasure' },
        ]},
        ],
      }));
    });

    // ----- Windows on south/north walls -----
    const winCfg = [
      { y: -halfD, side: 'South' }, { y: halfD, side: 'North' },
    ];
    winCfg.forEach(cfg => {
      [-8, -2, 4, 10].forEach((wx, wi) => {
        const w = add(makeElement({
          cls: 'IfcWindow', typeObj: types.window, predefined: 'WINDOW',
          name: `Window L${sIdx} ${cfg.side} ${wi + 1} — WT-EXT-W01`,
          storey, parent: storey,
          mat: 'glass',
          external: true,
          geometry: { kind: 'box', x: wx, y: cfg.y, z: z0 + 1.0, w: 1.4, d: 0.06, h: 1.6 },
          psets: types.window.propertySets.map(p => ({ ...p, properties: p.properties.slice() })),
          qtos: [{ name: 'Qto_WindowBaseQuantities', properties: [
            { name: 'Width', value: 1.4, unit: 'm', type: 'IfcLengthMeasure' },
            { name: 'Height', value: 1.6, unit: 'm', type: 'IfcLengthMeasure' },
            { name: 'Area', value: 2.24, unit: 'm²', type: 'IfcAreaMeasure' },
            { name: 'Perimeter', value: 6.0, unit: 'm', type: 'IfcLengthMeasure' },
          ]},
          ],
        }));
      });
    ...
    // ----- Doors -----
    // Front door on level 0, interior doors on each level
    if (sIdx === 0) {
      add(makeElement({
        cls: 'IfcDoor', typeObj: types.door, predefined: 'DOOR',
        ...
        qtos: [{ name: 'Qto_DoorBaseQuantities', properties: [
          ...
          { name: 'Area', value: 3.96, unit: 'm²', type: 'IfcAreaMeasure' },
        ]}],
      }));
    }
    // Interior doors
    [-4, 5].forEach(dx => {
      add(makeElement({
        ...
        qtos: [{ name: 'Qto_DoorBaseQuantities', properties: [
          ...
          { name: 'Area', value: 1.89, unit: 'm²', type: 'IfcAreaMeasure' },
        ]}],
      }));
    });
    ...
    zones.forEach(z => {
      const s = add(makeElement({
        cls: 'IfcSpace', typeObj: null, predefined: 'INTERNAL',
        name: `${z.name} L${sIdx}`,
        storey, parent: storey,
        mat: null,
        ...
        psets: [{ name: 'Pset_SpaceCommon', properties: [
          { name: 'IsExternal', value: false, type: 'IfcBoolean' },
          { name: 'GrossPlannedArea', value: z.w * z.d, unit: 'm²', type: 'IfcAreaMeasure' },
          { name: 'NetPlannedArea', value: z.w * z.d * 0.92, unit: 'm²', type: 'IfcAreaMeasure' },
        ...
        qtos: [{ name: 'Qto_SpaceBaseQuantities', properties: [
          ...
          { name: 'NetFloorArea', value: z.w * z.d, unit: 'm²', type: 'IfcAreaMeasure' },
          { name: 'NetVolume', value: z.w * z.d * storeyHeight, unit: 'm³', type: 'IfcVolumeMeasure' },
          { name: 'NetCeilingArea', value: z.w * z.d, unit: 'm²', type: 'IfcAreaMeasure' },
        ]}],
      }));
    });
  });
  ...
  add(makeElement({
    cls: 'IfcRoof', typeObj: null, predefined: 'FLAT_ROOF',
    name: 'Roof — Flat Roof Assembly',
    storey: storeys[storeys.length - 1], parent: storeys[storeys.length - 1],
    mat: 'slabAssembly',
    geometry: { kind: 'box', x: 0, y: 0, z: roofZ, w: W + 0.7, d: D + 0.7, h: 0.3 },
    psets: [{ name: 'Pset_RoofCommon', properties: [
      { name: 'IsExternal', value: true, type: 'IfcBoolean' },
      { name: 'FireRating', value: 'REI 90', type: 'IfcLabel' },
      { name: 'ThermalTransmittance', value: 0.18, unit: 'W/m²K', type: 'IfcThermalTransmittanceMeasure' },
    ]}],
    qtos: [{ name: 'Qto_RoofBaseQuantities', properties: [
      { name: 'GrossArea', value: (W + 0.7) * (D + 0.7), unit: 'm²', type: 'IfcAreaMeasure' },
      { name: 'NetArea', value: (W + 0.7) * (D + 0.7), unit: 'm²', type: 'IfcAreaMeasure' },
    ]}],
  }));

  // ----- Stair (one shared multi-storey stair) -----
  for (let i = 0; i < storeys.length; i++) {
    add(makeElement({
      cls: 'IfcStairFlight', typeObj: null, predefined: 'STRAIGHT',
      name: `Stair Flight L${i} → L${i + 1}`,
      storey: storeys[i], parent: storeys[i],
      mat: 'concrete',
  ...
  // ----- Add a couple of intentional data-quality issues -----
  // 1 wall with missing name on level 1
  const orphanWall = add(makeElement({
    cls: 'IfcWallStandardCase', typeObj: types.wallInt, predefined: 'STANDARD',
    name: '', // missing
    storey: storeys[1], parent: storeys[1],
    mat: 'intWallAssembly',
    external: false,
  ...
  // 1 element missing classification (we just won't add one for the roof above)

  // ----- Add classifications for elements that have them -----
  elements.forEach(el => {
  ...
  // ----- Compute raw attributes for each element -----
  elements.forEach(el => {
  ...
  return {
    ...
    fileSize: 18_420_113,
    totalEntities: elements.length + relationships.length + 24, // ~ entity count
...
function computeRawAttrs(el) {
  // Approximate the STEP/Express attribute layout: GlobalId, OwnerHistory,
  // Name, Description, ObjectType, ObjectPlacement, Representation, Tag, …
  const attrs = [
