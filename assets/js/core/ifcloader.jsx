/**
 * Real IFC loader powered by web-ifc (WASM).
 *
 * Lazily loads the bundled web-ifc-three runtime on first use, opens the user's file,
 * walks the IFC relations, extracts geometry, and produces a model object
 * with the exact shape the rest of the UI already expects.
 *
 * Perf notes
 * ----------
 * Critical path (blocks first render):
 *   ensureWebIfc → arrayBuffer → OpenModel → aggMap → containMap → typeByEl
 *   → entity scan → buildElement × N → StreamAllMeshes → bake
 *
 * Deferred path (starts 200 ms after model returns, transparent to inspector):
 *   psetLinksByEl → matLinksByEl → classLinksByEl → backfill _psetIds/_matIds/_classIds
 *
 * expandElement() awaits the deferred phase before reading those IDs,
 * so the inspector always works even if the user clicks an element quickly.
 */
...
const KNOWN_IFC_CLASSES = [
  ...
  'IfcPlate', 'IfcMember', 'IfcFooting', 'IfcPile', 'IfcReinforcingBar',
  'IfcReinforcingMesh', 'IfcTendon',
  // MEP
  'IfcDistributionElement', 'IfcFlowSegment', 'IfcFlowFitting', 'IfcFlowTerminal',
  ...
  'IfcAirTerminal', 'IfcAirTerminalBox', 'IfcOutlet', 'IfcSwitchingDevice',
  'IfcLightFixture', 'IfcSensor', 'IfcActuator', 'IfcController',
  'IfcAlarm', 'IfcAudioVisualAppliance', 'IfcCommunicationsAppliance',
  'IfcElectricAppliance', 'IfcSanitaryTerminal', 'IfcSpaceHeater',
  'IfcStackTerminal', 'IfcTransportElement', 'IfcWasteTerminal',
  // Furnishings
  'IfcFurnishingElement', 'IfcFurniture', 'IfcSystemFurnitureElement',
  // Catch-alls
  'IfcBuildingElementProxy', 'IfcElement', 'IfcElementAssembly',
  'IfcGeographicElement', 'IfcShadingDevice', 'IfcOpeningElement',
  'IfcChimney', 'IfcSurfaceFeature',
];

const TYPE_OBJECT_CLASSES = [
  'IfcWallType', 'IfcSlabType', 'IfcColumnType', 'IfcBeamType',
  'IfcDoorType', 'IfcDoorStyle', 'IfcWindowType', 'IfcWindowStyle',
  'IfcStairType', 'IfcStairFlightType', 'IfcRoofType', 'IfcCoveringType',
  'IfcRailingType', 'IfcMemberType', 'IfcPlateType', 'IfcFootingType',
  'IfcFurnitureType', 'IfcFurnishingElementType',
  'IfcBuildingElementProxyType',
  'IfcFlowSegmentType', 'IfcFlowFittingType', 'IfcFlowTerminalType',
  'IfcPipeSegmentType', 'IfcDuctSegmentType', 'IfcCableSegmentType',
  'IfcLightFixtureType', 'IfcOutletType',
];

// Translate "IfcWallStandardCase" -> the all-caps constant key web-ifc uses.
...
function ensureWebIfc(onLog) {
  ...
  _ifcPromise = (async () => {
    ...
    // Build a code->name map for every known IFC class.
...
async function loadRealIfc(file, opts = {}) {
  ...
  // Timing — populated throughout, logged before return.
  const _ts = {};
  const _mark = (n) => { _ts[n] = performance.now(); };
  ...
  onLog('ok', `Opened model · schema <span class="hl">${schemaVersion}</span>`);

  // ---- Critical-path relations: spatial hierarchy + type only ---------------
  // pset / mat / class links are deferred — see _buildSecondaryData below.
  ...
  // ---- Build element records (secondary link arrays start empty) ------------
  ...
  elementIds.forEach(id => {
    if (!byId.has(id)) {
      ...
      if (!cls) return;
      buildElement(id, cls, building?.expressId || project.expressId, null);
    }
  });
  ...
  api.StreamAllMeshes(modelID, (flatMesh) => {
    ...
    for (let i = 0; i < placed.size(); i++) {
      ...
      // Copy out of WASM memory before delete
      buffers.push({
  ...
  // ---- Relationships array (for inspector) ----------------------------------
  const relationships = [];
  let rid = 1;
  ...
  // ---- Timing log (critical path complete) ----------------------------------
  _mark('total_end');
  _logTiming(_ts, file);
  onProgress(1);

  // ---- Deferred secondary data ---------------------------------------------
  // pset / mat / class link indexes are built here in background.
  // expandElement() awaits this before reading _psetIds / _matIds / _classIds.
  ...
  const _buildSecondaryData = () => {
    ...
    _secPromise = (async () => {
      try {
        ...
        // Backfill into every element record
        byId.forEach(el => {
  ...
  // Start building secondary data shortly after first render
  setTimeout(_buildSecondaryData, 200);

  // ---- Lazy expand (inspector) ---------------------------------------------
  // We keep the model OPEN so expandElement can lazy-fetch on demand.
  async function expandElement(expressId) {
    const el = byId.get(expressId);
    if (!el) return null;
    if (el._expanded) return el;
    // Wait for secondary data so _psetIds / _matIds / _classIds are populated
    await _buildSecondaryData();
    if (el._expanded) return el; // may have been expanded while awaiting
    el._expanded = true;
      ...
      try {
        ...
        if (isQto) {
          for (const q of asArray(propDef.Quantities)) {
            let qLine;
            try { qLine = api.GetLine(modelID, val(q), true); } catch { continue; }
            const valueKey = ['LengthValue', 'AreaValue', 'VolumeValue', 'CountValue', 'WeightValue', 'TimeValue']
              .find(k => qLine[k] != null);
            const unit = { LengthValue: 'm', AreaValue: 'm²', VolumeValue: 'm³', CountValue: '', WeightValue: 'kg', TimeValue: 's' }[valueKey] || '';
          ...
          for (const p of asArray(propDef.HasProperties)) {
            let pLine;
            try { pLine = api.GetLine(modelID, val(p), true); } catch { continue; }
  ...
  // -------------------------------------------------------------------------
  // Local helpers (closures over modelID, api, W)
  // -------------------------------------------------------------------------

  function _logTiming(ts, f) {
    const d = (a, b) => (ts[a] != null && ts[b] != null)
      ? (ts[b] - ts[a]).toFixed(0) + ' ms'
      : '–';
    const rows = {
  ...
  function extractMaterial(line) {
      ...
      try {
        ...
        const layers = asArray(set.MaterialLayers).map(lh => {
          try {
            ...
            const matName = layer.Material ? (api.GetLine(modelID, val(layer.Material), true).Name?.value || '') : '';
            return { name: matName, thickness: (val(layer.LayerThickness) || 0) * 1000 };
          } catch { return null; }
        }).filter(Boolean);
    ...
    if (line.type === W.IFCMATERIALLIST) {
      return asArray(line.Materials).map(mh => {
        try { return { name: val(api.GetLine(modelID, val(mh), true).Name) || '' }; }
        catch { return { name: '?' }; }
      });
    }
...
// Yield control to the browser so the UI stays responsive during long ops.
function yieldUI() {
  return new Promise(r => setTimeout(r, 0));
}

// ---------------------------------------------------------------------------
// Combine multiple placed-geom buffers into one, applying web-ifc's placement
// transforms directly. The streamed geometry is already in viewer coordinates.
// ---------------------------------------------------------------------------
