(() => {
  ...
  class OrbitController {
    ...
    setView(name) {
      ...
      switch (name) {
        case "top":
          pos = new THREE.Vector3(t.x, t.y + r, t.z);
          break;
        case "bottom":
          pos = new THREE.Vector3(t.x, t.y - r, t.z);
          break;
        case "front":
          pos = new THREE.Vector3(t.x, t.y, t.z + r);
          break;
        case "back":
          pos = new THREE.Vector3(t.x, t.y, t.z - r);
          break;
        case "left":
          pos = new THREE.Vector3(t.x - r, t.y, t.z);
          break;
        case "right":
          pos = new THREE.Vector3(t.x + r, t.y, t.z);
          break;
        case "iso":
        default:
          pos = new THREE.Vector3(t.x + r * 0.6, t.y + r * 0.6, t.z + r * 0.6);
          break;
      }
  ...
  class ThreeViewer {
    constructor(host) {
      ...
      __publicField(this, "onPointerDown", (e) => {
        ...
        (_b = (_a = this.renderer.domElement).setPointerCapture) == null ? void 0 : _b.call(_a, e.pointerId);
      ...
      __publicField(this, "onPointerUp", (e) => {
        ...
        if (this._sectionDrag && (this._sectionDragPointer == null || e.pointerId === this._sectionDragPointer)) {
          ...
          try {
            (_b = (_a = this.renderer.domElement).releasePointerCapture) == null ? void 0 : _b.call(_a, e.pointerId);
          } catch (e2) {
          }
        }
      });
      ...
      this.camera = new THREE.PerspectiveCamera(45, 1, 0.1, 1e3);
      this.camera.position.set(28, 22, 28);
      this.camera.up.set(0, 1, 0);
      this.camera.lookAt(0, 4, 0);
      const hemi = new THREE.HemisphereLight(15265525, 9082536, 1.05);
      this.scene.add(hemi);
      this.hemiLight = hemi;
      const dir = new THREE.DirectionalLight(16777215, 1.35);
      dir.position.set(22, 38, 14);
      this.scene.add(dir);
      this.dirLight = dir;
      const dir2 = new THREE.DirectionalLight(12115199, 0.55);
      dir2.position.set(-18, 20, -12);
      this.scene.add(dir2);
      this.dirLight2 = dir2;
      const fill = new THREE.DirectionalLight(16774376, 0.28);
      fill.position.set(-12, 8, 24);
      this.scene.add(fill);
      this.fillLight = fill;
      this.grid = new THREE.GridHelper(80, 80, 9475743, 12962514);
      ...
      this.elementGeometryCenters = /* @__PURE__ */ new Map();
      this.elementOriginalPositions = /* @__PURE__ */ new Map();
      this.selectedIds = /* @__PURE__ */ new Set();
      this.hiddenIds = /* @__PURE__ */ new Set();
      this.isolatedIds = /* @__PURE__ */ new Set();
      this.hoverIds = /* @__PURE__ */ new Set();
      this.colorOverrides = /* @__PURE__ */ new Map();
      this.opacityOverrides = /* @__PURE__ */ new Map();
      ...
      this.sectionHandleMeshes = /* @__PURE__ */ new Map();
      this._sectionDrag = null;
      this._sectionDragPointer = null;
      this.annotations = [];
      this.measurePoints = [];
    ...
    _buildMaterials() {
      ...
      this.hoverEdgeMat = new THREE.LineBasicMaterial({ color: 16777215, transparent: true, opacity: 0.9 });
      this.selectEdgeMat = new THREE.LineBasicMaterial({ color: 7333887, linewidth: 2 });
    }
    _resolveElementColor(el, ifcColor) {
      ...
      if (ifcColor) {
        ...
        c.set((_a = el.colorOverride) != null ? _a : CLASS_COLOR(el.ifcClass));
      }
    ...
    matForElement(el) {
      var _a;
      const key = `${el.ifcClass}-${(_a = el.colorOverride) != null ? _a : CLASS_COLOR(el.ifcClass)}`;
      ...
      const m = new THREE.MeshStandardMaterial({
        ...
        opacity: isSpace ? 0.14 : isGlass ? 0.42 : 1,
    ...
    _ensureEdgeLines(mesh, id) {
      ...
      if (tri > 8e3) return null;
      const edgeGeo = new THREE.EdgesGeometry(mesh.geometry, 25);
      const edgeMat = new THREE.LineBasicMaterial({ color: 0, transparent: true, opacity: 0.45 });
    ...
    _pickAt(e) {
      ...
      const candidates = [];
      this.meshes.forEach((m) => {
        if (m.visible) candidates.push(m);
      });
    ...
    _applyColors() {
      ...
      this.meshes.forEach((m, id) => {
        ...
        const fakeEl = { ...el, colorOverride: override != null ? override : el.colorOverride };
    ...
    _applyViewMode() {
      ...
      this.meshes.forEach((m, id) => {
        ...
        switch (this.viewMode) {
          ...
          case "shaded":
          default: {
            const base = this._resolveElementColor(el, ((_a = el.geometry) == null ? void 0 : _a.color) || null);
            mat.color.copy(base);
            mat.emissive.copy(base).multiplyScalar(0.1);
            mat.emissiveIntensity = isSpace ? 0 : 0.42;
            mat.transparent = isSpace || isGlass;
            mat.opacity = isSpace ? 0.14 : isGlass ? 0.42 : 1;
    ...
    setShowEdges(v) {
      ...
      this.edgesGroup.children.forEach((c) => {
        var _a;
        c.visible = v && ((_a = this.meshes.get(c.userData.expressId)) == null ? void 0 : _a.visible) !== false;
      });
    }
    // ----- Camera helpers -----------------------------------------------------
    ...
    setExplode(factor) {
      ...
      this.meshes.forEach((m, id) => {
        ...
        const sIdx = (_a = storeyOrder.get(el.storey.expressId)) != null ? _a : 0;
    ...
    _applyTheme() {
      if (this.theme === "dark") {
        ...
        this._rebuildGrid(2765889, 1844012);
        this.hemiLight.color.set(13687526);
        this.hemiLight.groundColor.set(3818836);
        ...
        this._rebuildGrid(7042176, 12962514);
        this.hemiLight.color.set(15265525);
        this.hemiLight.groundColor.set(9082536);
    ...
    _rebuildGrid(colorCenter, colorGrid) {
      ...
      if (this.grid) {
        ...
        (_a = this.grid.geometry) == null ? void 0 : _a.dispose();
        if (Array.isArray(this.grid.material)) this.grid.material.forEach((m) => m.dispose());
        else (_b = this.grid.material) == null ? void 0 : _b.dispose();
      }
    ...
    _syncSectionVisuals(box) {
      ...
      if (!this.sectionBoxEdges) {
        const edgeMat = new THREE.LineBasicMaterial({
          color: this.theme === "dark" ? 16054008 : 2042163,
          transparent: true,
          opacity: 0.92,
          depthTest: false
        });
      ...
      this.sectionBoxEdges.material.color.setHex(this.theme === "dark" ? 16054008 : 2042163);
      ...
      if (enabled) {
        ...
        if (!this.clipHelper) {
          ...
          const outline = new THREE.LineSegments(
            ...
            new THREE.LineBasicMaterial({ color: 7333887, transparent: true, opacity: 0.4 })
          );
    ...
    _handleMeasureClick(e) {
      ...
      if (this.measurePoints.length === 2) {
        ...
        const mat = new THREE.LineBasicMaterial({ color: 7333887, linewidth: 2 });
